...
export function buildSpinePrompt(
  ...
  return [
    ...
    '',
    'ANTI-FUSION RULE (this is the single biggest correctness lever for',
    'Pass 1 — read carefully):',
    '',
    'If a sentence packages a CLAIM together with the EVIDENCE / NUMBERS /',
    'REASONING / MULTIPLIER / MECHANISM that justifies it, SPLIT them.',
    'In this pass emit ONLY the claim itself. Hold the supporting clause',
    'for Pass 2 to emit as a `fact`, `evidence`, or `assumption`. Do NOT',
    'fuse them into one node title or one node content blob.',
    '',
    'Decision rule:',
    '  - The CLAIM is the position the author is taking.',
    '  - Anything of the form "because X", "due to X", "after applying X",',
    '    "given that X" attached to the claim is SUPPORTING REASONING — it',
    '    is NOT part of the claim node. It belongs in Pass 2.',
    '  - If the sentence has BOTH the assertion AND the justification, the',
    '    claim node\'s title and content cover only the assertion.',
    '',
    'CONCRETE EXAMPLE (fixture-05 style):',
    '',
    '  Source sentence:',
    '    "Realistic 3-year build cost is $150-180k after 2x multiplier."',
    '',
    '  BAD (fused — DO NOT DO THIS):',
    '    {',
    '      "type": "claim",',
    '      "title": "3-year build cost is $150-180k after 2x multiplier",',
    '      "content": "Realistic 3-year build cost is $150-180k after 2x',
    '                  multiplier."',
    '    }',
    '',
    '  GOOD (split — Pass 1 emits the claim, Pass 2 will emit the fact):',
    '    Pass 1 (this pass):',
    '      {',
    '        "type": "claim",',
    '        "title": "3-year build cost is $150-180k",',
    '        "content": "Realistic 3-year build cost is $150-180k."',
    '      }',
    '    Pass 2 (next pass, NOT this one):',
    '      {',
    '        "type": "fact",',
    '        "title": "2x multiplier applied to base estimate for unknowns",',
    '        "content": "2x multiplier applied to the base build estimate to',
    '                    account for unknowns and overruns."',
    '      }',
    '',
    'The Pass-3 edge between them will be `supports: fact → claim`. That',
    'is how reasoning structure is preserved. Fused nodes destroy it.',
    '',
...
export function buildSupportPrompt(
  ...
  return [
    ...
    '',
    'PICK UP SPLIT-OFF JUSTIFICATIONS. Pass 1 was instructed to split',
    'fused claim+evidence sentences and emit ONLY the claim. The reasoning',
    'half ("after 2x multiplier", "because volume too low", "given that',
    'the cluster is undersized") is YOUR job to capture here as `fact`,',
    '`evidence`, or `assumption` — whichever fits. Look at each spine',
    'claim and ask: "what numbers / multipliers / mechanisms in the source',
    'are doing the work to justify this claim?" Emit those.',
    '',
