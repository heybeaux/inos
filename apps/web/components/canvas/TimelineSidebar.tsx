...
/**
 * TimelineSidebar — compact left sidebar showing the temporal structure.
 * Nodes are listed vertically by creation date with a "now" line
 * showing the current timeline slider position. Clicking focuses on 3D canvas.
 */
export function TimelineSidebar() {
  ...
  const nowY = timelineProgress; // 0-100
  ...
  return (
    <motion.div
      ...
      className="fixed top-14 left-0 bottom-0 w-60 z-20 overflow-y-auto p-3"
      style={{ background: 'rgba(10, 10, 26, 0.95)', borderRight: '1px solid var(--surface-glass-border)' }}
    >
      ...
      <div className="relative">
        <div className="absolute left-[11px] top-0 bottom-0" style={{ width: '2px', background: 'var(--surface-glass-border)' }} />
        ...
        {groups.map(([date, group]) => (
          <div key={date} className="relative mb-3">
            <div className="flex items-center gap-2 mb-1.5">
              <div className="w-2 h-2 rounded-full flex-shrink-0 z-10" style={{ background: 'var(--bio-amber)', marginLeft: '10px' }} />
              ...
              {group.map((node) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left px-2 py-1 rounded text-xs transition-all truncate"
                    style={{
      ...
      <div className="mt-4 pt-3 border-t text-[10px]" style={{ borderColor: 'var(--surface-glass-border)', color: 'var(--text-muted)' }}>
        ...
        <div className="flex justify-between">
          <span>Edges</span>
          <span style={{ color: 'var(--bio-cyan)' }}>
            {edges.filter((e) => visibleNodeIds ? visibleNodeIds.has(e.sourceId) && visibleNodeIds.has(e.targetId) : true).length} / {edges.length}
          </span>
        </div>
      </div>
    </motion.div>
